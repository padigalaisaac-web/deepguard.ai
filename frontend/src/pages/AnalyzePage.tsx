...
export const AnalyzePage: React.FC = () => {
  ...
  const handleStartAnalysis = async () => {
    if (!selectedFile) {
      setError(
        'Please select a media file to analyze.'
      );
      return;
    }
    ...
    if (!privacyConsent) {
      setError(
        'Please acknowledge the privacy & media rights consent checkbox.'
      );
      return;
    }
    ...
    try {
      ...
      const uploadResponse =
        await analysisService.uploadMedia(
          selectedFile
        );
      ...
      if (!analysisId) {
        throw new Error(
          'The backend uploaded the file but did not return an analysis ID. Please check the backend upload endpoint.'
        );
      }
      ...
      if (
        analysisResult.result === 'AUTHENTIC'
      ) {
        confetti({
          particleCount: 80,
          spread: 70,
      ...
      // Open result page only after a valid ID exists
      window.setTimeout(() => {
        navigate(
          `/analysis/${analysisId}`
        );
    ...
    <div className="p-6 sm:p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-400">
          Upload an image, video, or audio file for
          forensic inspection and synthetic
      ...
      {!isAnalyzing ? (
              ...
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80">
                <label className="flex items-start gap-3 cursor-pointer select-none">
                  ...
                  <span className="text-xs text-slate-300 leading-relaxed">
                    ...
                    I confirm that I have the legal
                    right or permission to analyze
                    this digital media. I understand
                    that processing is automated for
                    forensic purposes and temporary
                    files may be removed after
                    execution.
                  </span>
                </label>
              </div>
        ...
        <div className="p-10 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl backdrop-blur-xl space-y-8 text-center max-w-xl mx-auto">
          ...
          <span className="text-[11px] text-slate-400 block">
            Please wait while the forensic detectors
